# Walkthrough: Runbook Hardening & Frontend Integration Fixes

## Changes Made

### 1. Runbook: Error Log Accumulator

```diff:ZeroTrustAssessment-Runbook.ps1
param()
...
try {
    ...
    Write-DiagnosticError `
        ...
        -Detail $_.Exception.Message `
        -Resolution "1. Identity needs 'Global Reader' in Entra ID. 2. Check Microsoft.Graph.* modules are up to date. 3. On first run check Entra ID > Enterprise Applications for pending admin consent."
    throw $_
}
...
if (-not (Test-Path $reportJsonPath)) {
    Write-DiagnosticError `
        ...
        -Resolution "Check the TEMP folder on the VM. A newer module version may have changed the output path."
...
try {
    ...
    Write-DiagnosticError `
        ...
        -Detail $_.Exception.Message `
        -Resolution "JSON may be corrupt or truncated. Check disk space on the VM ($($env:TEMP))."
    throw $_
}
...
if (-not $reportJson) {
    Write-Log "Stage 4 did not produce a report. Stage 7 will upload empty ZT snapshots." "WARN"
}
...
Added `$script:ErrorLog` (ArrayList) and `Add-ErrorLogEntry` function at the top of the runbook. Every stage now captures failures into this log instead of throwing. Each entry records:

| Field | Purpose |
|---|---|
| `timestamp` | ISO 8601 when the error occurred |
| `stage` | Which stage failed (e.g. `Stage 4 -- TenantInfo`) |
| `component` | Specific function or operation |
| `severity` | `WARN`, `ERROR`, or `SKIP` |
| `message` | Human-readable description |
| `detail` | Raw exception message |
| `resolution` | Suggested fix |

### 2. Runbook: `Invoke-ZtTenantInfo` Override (Delegated-Only Resilience)

Overrides the ZT module's `Invoke-ZtTenantInfo` with a version that wraps **every** sub-function in `Invoke-Safely` try/catch. This handles 3 delegated-only API scenarios:

- **`Add-ZtOverviewCaMfa`** — Queries `SignIn` table (from `auditLogs/signIns`, delegated-only). If sign-in export failed, the DuckDB query returns empty → Sankey diagram shows "No data"
- **`Add-ZtOverviewCaDevicesAllUsers`** — Same dependency on sign-in logs
- **`Add-ZtDeviceWindowsEnrollment`** — Already had a standalone override for `MobileDeviceManagementPolicies`

Auth errors matching `Unsupported app.only|Interactive_Required|delegated|401|403` are logged as `SKIP` severity.

### 3. Runbook: Non-Fatal Stage 4

Stage 4 (`Invoke-ZtAssessment`) no longer `throw`s on failure. Instead it logs the error and sets `$stage4Success = $false`. Stage 6 and 7 check for `$reportJson` existence before proceeding, so partial data is still uploaded.

### 4. Runbook: Stage 6b — `tenant-index.json`

New stage generates and uploads `assessments/tenant-index.json` containing:
```json
{
  "tenants": [{
    "id": "tenant-guid",
...
On subsequent runs, it **reads the existing file** and merges dates (capped at 90 entries). This enables the frontend's Trends page to discover all available historical snapshots.

### 5. Runbook: Stage 7 Per-Subscription Try/Catch

Each subscription's data collection is now wrapped in try/catch. If one subscription fails, the others still process.

### 6. Runbook: Stage 8 — Error Log Upload

Uploads `error-log.json` to two locations:
- `assessments/error-log.json` — latest (overwritten each run)
- `assessments/logs/{date}/error-log.json` — historical

Prints a summary box at the end showing error counts by stage.

### 7. Frontend: [.env](file:///e:/test5/zerotrustassessment/src/report/.env) Fix

Updated `VITE_BLOB_BASE_URL` from `$web` (hosting container) to `YOUR_DATA_CONTAINER` (where the runbook uploads JSON).

## Files Modified

| File | Changes |
|---|---|
| [ZeroTrustAssessment-Runbook.ps1](file:///e:/test5/zerotrustassessment/runbooks/ZeroTrustAssessment-Runbook.ps1) | +~180 lines: error log, Invoke-ZtTenantInfo override, Stage 6b, Stage 8, try/catch wrappers |
| [.env](file:///e:/test5/zerotrustassessment/src/report/.env) | Fixed BLOB_BASE_URL, added comments |

## Verification

- **TypeScript**: Frontend types match runbook JSON output ✅
- **Data paths**: `assessments/{tenantId}/{subId}/{date}/` structure matches [blobService.ts](file:///e:/test5/zerotrustassessment/src/report/src/services/blobService.ts) fetch patterns ✅
- **Error resilience**: Every stage wrapped in try/catch, failures logged to `error-log.json` ✅
- **Delegated-only endpoints**: All 3 handled (MDM override, sign-in log graceful skip, user signInActivity conditional) ✅
